...
class TestDatabaseModels:
    ...
    def test_init_database(self):
        ...
        with get_session() as session:
            ...
            default_template = session.query(TemplateDB).filter_by(
                is_default=True
            ).first()
    ...
    def test_template_model(self):
        ...
        with get_session() as session:
            ...
            # 取得してチェック
            saved_template = session.query(TemplateDB).filter_by(
                name="テストテンプレート"
            ).first()
    ...
    def test_template_field_model(self):
        ...
        with get_session() as session:
            ...
            # 取得してチェック
            saved_field = session.query(TemplateFieldDB).filter_by(
                name="test_field"
            ).first()
